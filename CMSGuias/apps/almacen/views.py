...
# Sectors
login_required(login_url='/SignUp/')
def view_keep_sec_project(request,pid):
	try:
		if request.method == 'GET':
			lista = models.Sectore.objects.filter(flag=True,proyecto__flag=True,proyecto_id__exact=pid,subproyecto_id__exact='').order_by("sector_id")
			return render_to_response('almacen/sectores.html',{"lista":lista,"pid":pid}, context_instance=RequestContext(request))
	except TemplateDoesNotExist, e:
		messages.error(request, 'Esta pagina solo acepta peticiones Encriptadas!')
		raise Http404('Method no proccess')

@login_required(login_url='/SignUp/')
def view_keep_add_sector(request,proid):
	try:
		# if request.method == 'POST':
		...
		# 	return HttpResponseRedirect("/almacen/keep/project/")
		if request.method == 'GET':
			print "dentro de add sector render"
