...
var addManPower, addMaterials, calcPartitalMaterial, delMaterials, delMaterialsAll, editMaterials, getListMaterials, getManPowerAll, getMaterialsAll, getmeasure, getsummary, listManPower, openNewMaterial, refreshMaterials, showAddManPower, showAddMaterial, showEditMaterials;
...
listManPower = function(event) {
  ...
  $.getJSON("", context, function(response) {
    ...
    if (response.status) {
      $tb = $(".tmanpower > tbody");
      template = "{{#manpower}}<tr> <td>{{index}}</td> <td>{{code}}</td> <td>{{name}}</td> <td>{{unit}}</td> <td>{{gang}}</td> <td>{{quantity}}</td> <td>{{precio}}</td> <td>{{partial}}</td> <td><button class=\"btn btn-xs btn-warning\"> <span class=\"fa fa-edit\"></span> </button></td> <td><button class=\"btn btn-danger btn-xs\"><span class=\"fa fa-trash\"></span></button></td> </tr>{{/manpower}}";
      $b.empty();
      counter = 1;
      response.index + function() {
        return counter++;
      };
      return $tb.html(Mustache.render(template, response));
    } else {
