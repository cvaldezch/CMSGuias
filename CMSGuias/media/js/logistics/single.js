...
(function() {
  var addTmpQuote, changeRadio, deleteAll, deleteMaterial, editMaterial, finishTmp, keyCode, keyDescription, listTmpQuote, loadStore, loadSupplier, loadText, newQuote, saveQuote, searchMaterial, showEdit, showMaterials, stepSecond, uploadReadFile;
  ...
  saveQuote = function(event) {
    ...
    if (supplier.val() !== "" && store.val() !== "" && transfer.val() !== "") {
      ...
      $("[name=chk]").each(function() {
        if (this.checked) {
          mats.push({
            "materials_id": $(".table-quote > tbody > tr[name=" + this.value + "]").find("td").eq(2).html(),
            "quantity": $(".table-quote > tbody > tr[name=" + this.value + "]").find("td").eq(6).html()
          });
        }
      });
