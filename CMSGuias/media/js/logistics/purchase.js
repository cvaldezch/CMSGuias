...
var addTmpPurchase, blurRange, calcTotal, deleteAll, deleteMaterial, editMaterial, keyCode, keyDescription, listTmpBuy, saveOrderPurchase, searchMaterial, showBedside, showEdit, showMaterials, toggleDeposito, uploadReadFile;
...
listTmpBuy = function(event) {
  $.getJSON("", {
    ...
    var $tb, template, x;
    if (response.status) {
      template = "<tr name=\"{{ id }}\"> <td>{{ item }}</td><td>{{ materials_id }}</td> <td>{{ matname }}</td> <td>{{ matmeasure }}</td> <td>{{ unit }}</td> <td>{{ brand }}</td> <td>{{ brand }}</td> <td>{{ quantity }}</td> <td>{{ price }}</td> <td>{{ discount }}%</td> <td>{{ amount }}</td> <td><button class=\"btn btn-xs btn-link\" name=\"btn-edit\" value=\"{{ quantity }}\" data-price=\"{{ price }}\" data-brand=\"{{ brand }}\" data-model=\"{{ model }}\" data-id=\"{{ id }}\" data-mat=\"{{ materials_id }}\" data-discount=\"{{ discount }}\"><span class=\"glyphicon glyphicon-pencil\"></span></button></td> <td><button class=\"btn btn-xs btn-link text-red\" name=\"btn-del\" value=\"{{ id }}\" data-mat=\"{{ materials_id }}\"><span class=\"glyphicon glyphicon-trash\"></span></button></td> </tr>";
