  ...
  $scope.listNipple = ->
    ...
    .success (response) ->
      if response.status
        ...
        script = """{{#nip}}<tr class="text-12"><td class="center-align">{{index}}</td><td class="center-align">{{fields.cantidad}}</td><td class="center-align">{{fields.cantshop}}</td><td class="center-align">{{fields.tipo}}</td><td>Niple {{#desc}}{{>fields.tipo}}{{/desc}}</td><td>{{fields.materiales.fields.matmed}}</td><td>x</td><td>{{fields.metrado}} cm</td><td>{{fields.comment}}</td><td><a href="#" ng-click="nedit($event)" data-pk="{{pk}}" data-materials="{{fields.materiales.pk}}" ng-if="perarea == 'administrator' || perarea == 'ventas' || perarea == 'operaciones'"><i class="fa fa-edit"></i></a></td><td><a href="#" ng-click="ndel($event)" data-pk="{{pk}}" data-materials="{{fields.materiales.pk}}" class="red-text text-darken-1" ng-if="perarea == 'administrator' || perarea == 'ventas' || perarea == 'operaciones'"><i class="fa fa-trash"></i></a></td></tr>{{/nip}}"""
