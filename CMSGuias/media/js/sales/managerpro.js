...
var LetterAnular, addDetailsPurchase, approvedProject, assignedResponsible, calAmountPurchaseOrder, changeView, deleteDetailsPurchase, deleteOrderPurchase, deleteSubproject, editLetter, fileTree, genKeyConfirmationApproved, genKeyConfirmationResponsible, generateLetter, getAmountLiteral, getEditPurchase, getListLetter, getListPurchaseOrder, getPercentIVA, getSectors, loadsAccounts, openNewSector, openNewSubproyecto, openUpdateSector, openUpdateSubproject, openWindow, publisherCommnet, savePurchaseOrder, setSubproject, showAlertStartProject, showEditComment, showEditDetailsPurchase, showGuideByProject, showLetter, showLetterAnexosUpload, showLetterEdit, showLetterObservation, showLetterUpload, showLoadPrices, toggleComment, treeAdminaandOpera, uploadFiles, uploadLetter, uploadLetterAnexo, uploadLoadPrices;
    ...
    if (response.status) {
      ...
      for (x in response.list) {
        ...
        if (response.list[x].file.length) {
          tmp = tmp.replace("{{!files}}", "<a href=\"/media/{{ file }}\" class=\"text-black\" target=\"_blank\"> <span class=\"fa fa-file-text\"></span> </a>");
        }
...
uploadLetterAnexo = function(event) {
  var $file, context, i, _i, _ref;
  $file = $("input[name=letter-anexos-file]");
  if ($file.get(0).files.length) {
    context = new FormData;
    context.append("id", this.value);
    for (i = _i = 0, _ref = $file.get(0).files.length; _i < _ref; i = _i += 1) {
