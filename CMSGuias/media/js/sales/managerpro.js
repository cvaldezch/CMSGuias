...
var addDetailsPurchase, approvedProject, assignedResponsible, calAmountPurchaseOrder, changeView, deleteDetailsPurchase, deleteOrderPurchase, deleteSubproject, fileTree, genKeyConfirmationApproved, genKeyConfirmationResponsible, generateLetter, getAmountLiteral, getEditPurchase, getListLetter, getListPurchaseOrder, getPercentIVA, getSectors, loadsAccounts, openNewSector, openNewSubproyecto, openUpdateSector, openUpdateSubproject, openWindow, publisherCommnet, savePurchaseOrder, setSubproject, showAlertStartProject, showEditComment, showEditDetailsPurchase, showGuideByProject, showLetter, showLoadPrices, toggleComment, treeAdminaandOpera, uploadFiles, uploadLoadPrices;
...
getListLetter = function(event) {
  ...
  $.getJSON("", context, function(response) {
    var template;
    if (context.status) {
      return template = "<tr> <td>{{ item }}</td> <td>{{ code }}</td> <td>{{ form }}</td> <td>{{ to }}</td> <td>{{ status }}</td> <td> <div class=\"btn-group\"> <button type=\"button\" data-toggle=\"dropdown\" aria-expanded=\"false\" class=\"btn btn-xs btn dropdown-toggle\"> <span class=\"fa fa-wrench\"></span> <span class=\"caret\"></span> </button> <ul class=\"dropdown-menu\" role=\"menu\"> <li><a href=\"\">Subir Carta</a></li> <li><a href=\"\">Subir Anexos</a></li> </ul> </div> </td> </tr>";
    } else {
