...
var addMaterial, aggregate_nipples, changeRadio, changeRdoNip, copyBack, delMaterials, delPlane, delete_all_temp_nipples, deleteallnipmat, dellAllMaterial, editBrandandModel, editMaterials, listMaterials, list_temp_nipples, loadMaterials, loadSecandSub, loadSector, openAddMaterial, openBrand, openModel, panelPlanes, pasteMaterials, saved_or_update_nipples, selectChoiseOrder, showHideTbody, showListNipp, showOrders, show_edit_nipple, uploadPlane, viewFull;
  ...
  if (counter > 0) {
    ...
    for (x in data.list) {
      ...
      if ($.trim(data.list[x].materials.substring(0, 3)) === "115") {
        template = template.replace("{{! input }}", "<div class=\"input-group\"> <input type=\"number\" class=\"form-control input-sm\" readonly=\"readonly\"> <span class=\"input-group-btn\"> <button class=\"btn btn-default btn-sm btn-append-list-nipp\" value=\"{{ materials }}\" type=\"button\"> <span class=\"glyphicon glyphicon-list\"></span> </button> </span> </div>");
      } else {
        template = template.replace("{{! input }}", "<input type=\"number\" min=\"1\" max=\"{{ quantity }}\" value=\"{{ quantity }}\" class=\"form-control input-sm\">");
      }
...
showListNipp = function() {
  ...
  arr = new Array();
  counter = 0;
  if (!$(".torders > tbody.nipples > tr.prenip" + this.value).length) {
...
showHideTbody = function() {
  ...
  if (this.value.length === 15) {
    ...
    $("." + this.value + " > tbody").toggle(function() {
      if ($(this).is(":hidden")) {
        return $(btn).find("span").removeClass("glyphicon-chevron-up").addClass("glyphicon-chevron-down");
      } else {
        return $(btn).find("span").removeClass("glyphicon-chevron-down").addClass("glyphicon-chevron-up");
