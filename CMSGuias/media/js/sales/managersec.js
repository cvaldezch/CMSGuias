...
var addListCusSectors, addMaterial, addMaterialUpdateMeter, addoldMaterialRemoveDeductive, aggregateMaterialsOutMeter, aggregate_nipples, approvedAdditional, approvedModify, backModify, backOrders, calMeter, changeRadio, changeRdoNip, changeSelectDeductiveMeter, changeTypeDeductive, chkNippChange, clearFieldsDeductiveMeter, copyBack, createTableDeductive, deductiveOneCancel, delMaterials, delPlane, deleteMaterialUpdateMeter, delete_all_temp_nipples, deleteallnipmat, dellAllMaterial, displayResultTable, editBrandandModel, editMaterials, generateDeductiveMeter, generateOrders, listMaterials, list_temp_nipples, loadMaterials, loadSecandSub, loadSector, nextOrders, openAddMaterial, openBrand, openModel, panelPlanes, pasteAllLeft, pasteAllRight, pasteMaterials, pasteOneLeft, pasteOneRight, saved_or_update_nipples, searchDescDeductiveGlobal, selectChoiseOrder, showHideTbody, showInitDeductive, showListNipp, showModify, showOrders, showPanelAddMateialsOldDeductiveGlobal, show_edit_nipple, showaddtableoutdeductivemeter, startModidfy, updateCommentMat, updateMaterialUpdateMeter, uploadPlane, valMax, valQuantityPreOrders, validBlurNumber, validOrders, viewFull;
  ...
  for (x in tblb) {
    ...
    if (tbla.length === count) {
      ...
      if (qmodify > qoriginal) {
        ...
        if (quantityorders > 0 && quantityorders < qoriginal) {
          tblb[x].tag = "1";
        }
        ...
        if (qmodify > 0 && quantityorders === qoriginal) {
          tblb[x].tag = "0";
        } else if (qmodify > 0 && quantityorders < qoriginal && quantityorders > 0) {
          tblb[x].tag = "1";
...
addoldMaterialRemoveDeductive = function(event) {
  ...
  data = {
    "mateials": this.getAttribute("data-id"),
    ...
    "amount": parseFloat(this.getAttribute("data-quanity")) * parseFloat(this.getAttribute("data-price"))
  };
  template = "<tr> <td>{{ item }}</td> <td>{{ materials }}</td> <td>{{ name }}</td> <td>{{ measure }}</td> <td>{{ unit }}</td> <td>{{ quantity }}</td> <td>{{ price }}</td> <td>{{ amount }}</td> <td>Sectores</td> </tr>";
